...
/// rename MET correction for data
ROOT::RDF::RNode renameMET(ROOT::RDF::RNode df, const std::string &outputname,
                                 const std::string &met_p4_uncorr) {
    auto rename_p4 = [](ROOT::Math::PtEtaPhiMVector &met_p4_uncorr) {
        return met_p4_uncorr;
    };
    auto df1 = 
        df.Define(outputname, rename_p4, {met_p4_uncorr});
    return df1;
}
///
...
///
/// function to pick dimuon pair in DY control region High mass
ROOT::RDF::RNode DY_DiMuonPair_CR_HighMass(ROOT::RDF::RNode df, const std::string &outputname,
                                 const std::string &particle_pts,
                                 const std::string &particle_etas,
                                 const std::string &particle_phis,
                                 const std::string &particle_masses,
                               ...
                               const ROOT::RVec<int> &goodmuons_index) {
                                 ...
                                 for (unsigned int i = 0; i < p4_1.size(); ++i) {
                                     for (unsigned int j = i + 1; j < p4_2.size(); ++j) {
                                         ...
                                         /// Add dimuon mass window, 70-110, 150-Inf
                                         if ( ( (p4_1[i] + p4_2[j]).mass() < 70 ) || ( (p4_1[i] + p4_2[j]).mass() > 110 && (p4_1[i] + p4_2[j]).mass() < 150 ) ) {
                                             continue;
                                         }
                                         if ( p4_1[i].pt() + p4_2[j].pt() > ptsum) {
                                             ptsum = p4_1[i].pt() + p4_2[j].pt();
...
///help from Zhiyuan: electron scale and smearing. Add by Mingxuan
    ...
    // usage of unsigned int is critical, the lenght of data_run is not the same as the other column
        [evaluator, jsonESname](const unsigned int &data_run, 
            ...
            for (int i = 0; i < pt.size(); i++) {
                ...
                if (pt[i] > 20 && pt[i] < 100) { // only applying scale correction to electrons in the pt range of 20 to 200 GeV, as recommended by the EGamma POG
                    corrected_pt_values[i] = pt[i] * sf;
                }
                else {
                    corrected_pt_values[i] = pt[i];
                }
            }
            return corrected_pt_values;
        };
...
PtCorrection_smearing(ROOT::RDF::RNode df, const std::string &corrected_pt,
    ...
    auto electron_pt_correction_lambda =
        [evaluator, Smear_variation, cset_rndmseed](const ROOT::RVec<float> &pt,  
                                        ...
                                        ULong64_t event, UInt_t luminosityBlock) {
            ROOT::RVec<float> corrected_pt_values(pt.size());
