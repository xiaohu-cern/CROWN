...
inline double CalcMT(double px1, double px2, double py1, double py2, double m1 , double m2){
...
inline std::pair <double,double>  ben_findsols(double MT2, double px, double py, double visM, double Ma, double pxb, double pyb, double metx, double mety, double visMb, double Mb){
  ...
  if (TermSqy1*TermSqy1-4.*TermSqy0*TermSqy2 < 0){
    ...
    for (double metpy = low; metpy<=high; metpy+=(high-low)/10000.){
      ...
      double mt1a = CalcMT(px,metpx,py,metpy,visM,Ma);
      double mt1b = CalcMT(px,metpx2,py,metpy,visM,Ma);
      double metpxb = metx-metpx;
      double metpx2b = metx-metpx2;
      double mt2a = CalcMT(pxb,metpxb,pyb,mety-metpy,visMb,Mb);
      double mt2b = CalcMT(pxb,metpx2b,pyb,mety-metpy,visMb,Mb);
